...
class ProposalAkadController extends Controller
{
    ...
    public function index()
    {
        $proposalpasar = PasarPembiayaanHistory::select()
            ...
            ->where('status_id', 5)
            ->whereIn('jabatan_id', [2, 4])
            // ->orWhere('jabatan_id', 4)
        ...
        $proposalP3k = P3kPembiayaanHistory::select()
    ...
    {
        $jmlPembiayaan = Pembiayaan::select()->count();

        if ($jmlPembiayaan == 0) {
            $hitungId = Pembiayaan::select()->count();
            $id = $hitungId + 1;
        } else {
            $hitungId = Pembiayaan::select()->orderBy('id', 'DESC')->first();
            $id = $hitungId->id + 1;
        }
    ...
    public function showP3k($id)
    {
        ...
        return view('akad::proposal.lihat', [
            ...
            'hargaJual' => $hargaJual,
            'hargaJualTerbilang' => $hargaJualTerbilang,
            'tenor' => $tenor,
            'rate' => $rate,
            'margin' => $margin,
            'angsuran' => $angsuran,
