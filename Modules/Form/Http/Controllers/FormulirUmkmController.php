...
class FormulirUmkmController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $hitungid = UmkmPembiayaan::select()->count();
        if ($hitungid > 0) {
            $hitung = UmkmPembiayaan::select()->orderby('created_at', 'desc')->first();
            $id = $hitung->id + 1;
        } else {
            $id = $hitungid + 1;
        }
        ...
        $formatTglPembiayaan = preg_match('/^\d{4}-\d{2}-\d{2}$/', $request->tgl_pembiayaan) ? 'Y-m-d' : 'd-m-Y';
        $formatTglLahir = preg_match('/^\d{4}-\d{2}-\d{2}$/', $request->tgl_lahir) ? 'Y-m-d' : 'd-m-Y';
        $tglPembiayaan = Carbon::createFromFormat($formatTglPembiayaan, $request->tgl_pembiayaan)->format('Y-m-d');
        $tglLahir = Carbon::createFromFormat($formatTglLahir, $request->tgl_lahir)->format('Y-m-d');
    ...
    public function edit($id)
    {
        ...
        return view('form::umkm.nasabah', [
            'title' => 'Form Pasar',
            'pembiayaan' => UmkmPembiayaan::select()->where('id', $id)->first(),
            'nasabah' => UmkmNasabah::select()->where('id', $pembiayaan->id)->first(),
