...
class FormPprController extends Controller
{
    ...
    {
        $jmlPembiayaan = FormPprPembiayaan::select()->count();
        $jmlClFixed = PprClDokumenFixedIncome::select()->count();
        $jmlClNonFixed = PprClDokumenNonFixedIncome::select()->count();
        ...
        if ($jmlPembiayaan == 0) {
            $hitung =  FormPprPembiayaan::select()->count();
            $id = $hitung + 1;

            //Id Check List
            $hitungIdClDokumenFixed = PprClDokumenFixedIncome::select()->count();
            $idClDokumenFixed = $hitungIdClDokumenFixed + 1;

            $hitungIdClAtrFixed = PprAbilityToRepayFixedIncome::select()->count();
            $idClAtrFixed = $hitungIdClAtrFixed + 1;

            $hitungIdClDokumenNonFixed = PprClDokumenNonFixedIncome::select()->count();
            $idClDokumenNonFixed = $hitungIdClDokumenNonFixed + 1;

            $hitungIdClAtrNonFixed = PprAbilityToRepayNonFixedIncome::select()->count();
            $idClAtrNonFixed = $hitungIdClAtrNonFixed + 1;

            //Id Scoring
            $hitungIdScoringAtrFixed = PprScoringAtrFixedIncome::select()->count();
            $idScoringAtrFixed = $hitungIdScoringAtrFixed + 1;

            $hitungIdScoringWtrFixed = PprScoringWtrFixedIncome::select()->count();
            $idScoringWtrFixed = $hitungIdScoringWtrFixed + 1;

            $hitungIdScoringCcFixed = PprScoringCollateralFixedIncome::select()->count();
            $idScoringCcFixed = $hitungIdScoringCcFixed + 1;

            $hitungIdScoringAtrNonFixed = PprScoringAtrNonFixedIncome::select()->count();
            $idScoringAtrNonFixed = $hitungIdScoringAtrNonFixed + 1;

            $hitungIdScoringWtrNonFixed = PprScoringWtrNonFixedIncome::select()->count();
            $idScoringWtrNonFixed = $hitungIdScoringWtrNonFixed + 1;

            $hitungIdScoringCcNonFixed = PprScoringCollateralNonFixedIncome::select()->count();
            $idScoringCcNonFixed = $hitungIdScoringCcNonFixed + 1;
        } else {
            $hitung = FormPprPembiayaan::select()->orderBy('id', 'DESC')->first();
            $id = $hitung->id + 1;
            ...
            if ($jmlClFixed == 0) {
                ...
                //Id Check List
                $hitungIdClDokumenFixed = PprClDokumenFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idClDokumenFixed = $hitungIdClDokumenFixed->id + 1;

                $hitungIdClAtrFixed = PprAbilityToRepayFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idClAtrFixed = $hitungIdClAtrFixed->id + 1;

                //Id Score
                $hitungIdScoringAtrFixed = PprScoringAtrFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idScoringAtrFixed = $hitungIdScoringAtrFixed->id + 1;

                $hitungIdScoringWtrFixed = PprScoringWtrFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idScoringWtrFixed = $hitungIdScoringWtrFixed->id + 1;

                $hitungIdScoringCcFixed = PprScoringCollateralFixedIncome::select()->orderBy('id', 'DESC')->first();
            ...
            if ($jmlClNonFixed == 0) {
                ...
                //Id Check List
                $hitungIdClDokumenNonFixed = PprClDokumenNonFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idClDokumenNonFixed = $hitungIdClDokumenNonFixed->id + 1;

                $hitungIdClAtrNonFixed = PprAbilityToRepayNonFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idClAtrNonFixed = $hitungIdClAtrNonFixed->id + 1;

                //Id Score
                $hitungIdScoringAtrNonFixed = PprScoringAtrNonFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idScoringAtrNonFixed = $hitungIdScoringAtrNonFixed->id + 1;

                $hitungIdScoringWtrNonFixed = PprScoringWtrNonFixedIncome::select()->orderBy('id', 'DESC')->first();
                $idScoringWtrNonFixed = $hitungIdScoringWtrNonFixed->id + 1;

                $hitungIdScoringCcNonFixed = PprScoringCollateralNonFixedIncome::select()->orderBy('id', 'DESC')->first();
    ...
    public function edit($id)
    {
        return view('form::ppr.nasabah', [
            ...
            // 'kekayaan_simpanan' => FormPprDataKekayaanSimpanan::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'kekayaan_tanah_bangunan' => FormPprDataKekayaanTanahBangunan::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'kekayaan_kendaraan' => FormPprDataKekayaanKendaraan::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'kekayaan_saham' => FormPprDataKekayaanSaham::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'kekayaan_lainnya' => FormPprDataKekayaanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'pinjaman' => FormPprDataPinjaman::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'pinjaman_kartu_kredit' => FormPprDataPinjamanKartuKredit::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'pinjaman_lainnya' => FormPprDataPinjamanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->first(),
