...
class UmkmKomiteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('dokumen_deviasi')) {
            $dokumen_deviasi = $request->file('dokumen_deviasi')->store('umkm-dokumen_deviasi', 'public');
            UmkmDeviasi::create([
                'umkm_pembiayaan_id' => $request->umkm_pembiayaan_id,
    ...
    public function upload(Request $request)
    {
        if ($request->file('Foto_Bon_Murabahah')) {
            $bonmurabahah = $request->file('Foto_Bon_Murabahah')->store('foto-umkm-pembiayaan', 'public');
            UmkmFoto::create([
                'umkm_pembiayaan_id' => $request->umkm_pembiayaan_id,
