...
class UmkmRevisiController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('umkm::revisi.lihat', [
            ...
            'aos' => Role::select()->where('jabatan_id', 1)->where('divisi_id', 3)->get(),
            ...
            'jaminanutama' => UmkmJaminan::select()->where('umkm_pembiayaan_id', $id)->first(), //udah
            'jaminanlain' => UmkmJaminanLain::select()->where('umkm_pembiayaan_id', $id)->first(), //udah
            ...
            // 'fotodiriktp' => UmkmFoto::select()->where('umkm_pembiayaan_id', $id)->where('kategori', 'Foto Diri Bersama KTP')->first(),
    ...
    public function update(Request $request, $id)
    {
        ...
        UmkmPembiayaan::where('id', $id)->update([
            ...
            'ket_pendapatan_lain' => $request->ket_pendapatan_lain,
            'slik_id' => $id,
        ...
        if ($request->file('dokumen_keuangan')) {
            ...
            $dokumen_keuangan = $request->file('dokumen_keuangan')->store('umkm-dokumen-keuangan', 'public');
            UmkmPembiayaan::where('id', $id)->update([
        ...
        if ($request->file('dokumenktb')) {
            if ($request->dokumenlama) {
                Storage::disk('public')->delete($request->dokumenlama);
            }
            $dokumenktb = $request->file('dokumenktb')->store('umkm-dokumen-ktb', 'public');
        ...
        if ($request->file('dokumen_jaminan')) {
            if ($request->dokumenjaminanlama) {
                Storage::disk('public')->delete($request->dokumenjaminanlama);
            }

            $dokumen_jaminan = $request->file('dokumen_jaminan')->store('umkm-dokumen_jaminan', 'public');
        ...
        //     'foto.*.foto' => 'required',
        // ]);
        if (request('perbarui_lampiran_pemohon') == 'Ya') {
            foreach ($request->foto as $key => $value) {
                if ($value['foto']) {
                    Storage::disk('public')->delete($value['foto_lama']);
                    $foto = $value['foto']->store('foto-umkm-pembiayaan', 'public');
