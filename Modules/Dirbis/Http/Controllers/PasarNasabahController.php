...
class PasarNasabahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('dirbis::pasar.nasabah.index', [
            'title' => 'Nasabah',
            'proposals' => PasarNasabahh::select()->when($search, fn($q) => $q->where('nama_nasabah', 'like', "%{$search}%"))->paginate(10)->withQueryString(),
        ]);
    }
    ...
    public function show($id)
    {
        ...
        return view('dirbis::pasar.nasabah.lihat', [
            'title' => 'Nasabah',
            'pembiayaan' => PasarPembiayaan::select()->where('nasabah_id', $id)->first(),
            'nasabah' => $nasabah,
