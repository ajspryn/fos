...
namespace Modules\Dirbis\Providers;
...
class DirbisServiceProvider extends ServiceProvider
{
    /**
     * @var string $moduleName
     */
    protected $moduleName = 'Dirbis';

    /**
     * @var string $moduleNameLower
     */
    protected $moduleNameLower = 'dirbis';
    ...
    public function boot()
    {
        ...
        View::composer('dirbis::layouts.sidebar', function ($view) {
            $data = Cache::remember('dirbis:sidebar:stats', 300, function () {
                ...
                $proposalppr = DB::query()
                    ...
                    ->where(function ($q) {
                        $q->where(function ($q2) {
                            $q2->where('prh.jabatan_id', 3)->where('prh.status_id', 5);
                        })->orWhere(function ($q2) {
                            $q2->where('prh.jabatan_id', 4)->where('prh.status_id', 4);
                        });
                    })
                    ->count();
                ...
                $proposalP3k = DB::query()
                    ...
                    ->where(function ($q) {
                        $q->where(function ($q2) {
                            $q2->where('p3h.jabatan_id', 3)->where('p3h.status_id', 5);
                        })->orWhere(function ($q2) {
                            $q2->where('p3h.jabatan_id', 4)->where('p3h.status_id', 4);
                        });
                    })
                    ->count();

                return compact('proposalSkpd', 'data', 'b', 'proposalppr', 'proposalP3k');
            });
