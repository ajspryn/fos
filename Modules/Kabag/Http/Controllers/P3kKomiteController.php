...
class P3kKomiteController extends Controller
{
    ...
    //     $proposals = P3kPembiayaanHistory::with([
    ...
    //         })
    //         ->where('created_at', '>=', Carbon::now()->subMonths(3)) // Filter for data within the last 3 months
    //         ->groupBy('p3k_pembiayaan_id')
    //         ->orderBy('updated_at', 'desc')
    //         ->get();
    ...
    public function index(Request $request)
    {
        ...
        // Main query to fetch the proposals with required conditions
        $proposals = P3kPembiayaanHistory::with([
            'p3kPembiayaan.nasabah.pekerjaan',
            'statusHistory',
            'jabatan',
            ...
            ->whereNot(function ($query) {
                $query->where(function ($q2) {
                    $q2->where('status_id', 11)->where('jabatan_id', 3);
                })->orWhere(function ($q2) {
                    $q2->where('status_id', 4)->where('jabatan_id', 2);
                });
            })
    ...
    public function store(Request $request)
    {
        ...
        if ($request->status_id == 5) {
            return redirect('/kabag/p3k/komite/')->with('success', 'Proposal Berhasil Disetujui!');
        } elseif ($request->status_id == 6) {
            return redirect('/kabag/p3k/komite/')->with('success', 'Proposal Berhasil Ditolak!');
        } elseif ($request->status_id == 7) {
            return redirect('/kabag/p3k/komite/')->with('success', 'Proposal Diajukan Untuk Revisi!');
        } else {
        }
    }
    ...
    public function show($id)
    {
        ...
        //DSR
        $dsr = $totalPendapatan
            ? number_format((($angsuran + $totalAngsuranBtbFasAktif) / $totalPendapatan) * 100)
            : 0;
        //DSR Join Income
        $dsrJoinIncome = $totalPendapatanJoinIncome
            ? number_format((($angsuran + $totalAngsuranBtbFasAktif) / $totalPendapatanJoinIncome) * 100)
            : 0;

        //Usia
        $usia = $data->nasabah && $data->nasabah->tgl_lahir
            ? Carbon::parse($data->nasabah->tgl_lahir)->age
            : 0;
        $usiaInMonth = $usia * 12;
        ...
        $historyStatus = P3kPembiayaanHistory::select()
            ->where('p3k_pembiayaan_id', $id)
            ->orderby('created_at', 'desc')
            ->first();

        return view('kabag::p3k.komite.lihat', [
