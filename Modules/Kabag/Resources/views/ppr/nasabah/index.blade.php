    ...
    <div class="app-content content ">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <section id="basic-datatable">
                ...
                <form method="GET" action="/kabag/ppr/nasabah" class="d-flex gap-2">
                    <input type="text" name="search" class="form-control" placeholder="Cari nama pemohon..." value="{{ request('search') }}">
                    <button type="submit" class="btn btn-primary">Cari</button>
                    <a href="/kabag/ppr/nasabah" class="btn btn-secondary">Reset</a>
                </form>
            </div>
                                    ...
                                    <tbody class="table-border-bottom-0">
                                        @forelse ($proposals as $proposal)
                                            <tr>
                                                ...
                                                <td class="d-none d-md-table-cell">{{ $proposal->form_pribadi_pemohon_nama_lengkap ?? '-' }}</td>
                                                <td style="text-align: center">{{ $proposal->form_pribadi_pemohon_no_ktp ?? '-' }}</td>
                                                <td>
                                                    {{ $proposal->form_pribadi_pemohon_alamat_ktp ?? '-' }}
                                                    @if (!empty($proposal->form_pribadi_pemohon_alamat_ktp_rt) || !empty($proposal->form_pribadi_pemohon_alamat_ktp_rw))
                                                        RT {{ $proposal->form_pribadi_pemohon_alamat_ktp_rt ?? '-' }},
                                                        RW {{ $proposal->form_pribadi_pemohon_alamat_ktp_rw ?? '-' }}
                                                    @endif
                                                    @if (!empty($proposal->form_pribadi_pemohon_alamat_ktp_kelurahan) || !empty($proposal->form_pribadi_pemohon_alamat_ktp_kecamatan))
                                                        , KEL. {{ $proposal->form_pribadi_pemohon_alamat_ktp_kelurahan ?? '-' }}
                                                        , KEC. {{ $proposal->form_pribadi_pemohon_alamat_ktp_kecamatan ?? '-' }}
                                                    @endif
                                                </td>
                                                <td style="text-align: center">{{ $proposal->form_pribadi_pemohon_no_handphone ?? '-' }}</td>
                                                <td style="text-align: center">
                                                    <a href="/kabag/ppr/nasabah/{{ $proposal->id }}" class="btn btn-outline-info btn-sm round">Detail</a>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
