...
use Modules\Ppr\Http\Controllers\PprKomiteController;
...
Route::prefix('ppr')->middleware(['auth:sanctum', 'verified', 'role:2', 'divisi:4', 'jabatan:1'])->group(function () {
    ...
    // });
    Route::resource('/proposal', PprProposalController::class)->names('ppr.proposal');

    Route::resource('/komite', PprKomiteController::class)->names('ppr.komite');
    Route::resource('/nasabah', PprNasabahController::class)->names('ppr.nasabah');
    Route::resource('/revisi', PprEditProposalController::class)->names('ppr.revisi');
