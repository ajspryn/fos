...
class PprEditProposalController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $proposals = FormPprPembiayaan::with(['pemohon', 'user'])
            ...
            ->where(function ($query) {
                $query
                    ->orWhere('dilengkapi_ao', 'Butuh Revisi')
    ...
    {
        $dataFoto = FormPprFoto::select()->where('form_ppr_pembiayaan_id', $id)->get();
        $foto = $dataFoto;
        return view('ppr::revisi.lihat', [
            'title' => 'Revisi Proposal PPR',
            'pembiayaan' => FormPprPembiayaan::select()->where('id', $id)->first(),
            'dokumen' => PprClDokumen::select()->where('id', $id)->first(),

            'clPersyaratan' => PprClPersyaratan::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'dokFixedIncome' => PprClDokumenFixedIncome::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'dokNonFixedIncome' => PprClDokumenNonFixedIncome::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'dokAgunan' => PprClDokumenAgunan::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'AtrFixedIncome' => PprAbilityToRepayFixedIncome::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'AtrNonFixedIncome' => PprAbilityToRepayNonFixedIncome::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'pemberkasanMemo' => PprPemberkasanMemo::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'nasabah' => FormPprDataPribadi::select()->where('id', $id)->first(),
            'fotoPemohon' => FormPprFoto::select()->where('form_ppr_pembiayaan_id', $id)->where('kategori', 'Foto Pemohon')->first(),
            'fotoPasanganPemohon' => FormPprFoto::select()->where('form_ppr_pembiayaan_id', $id)->where('kategori', 'Foto Pasangan Pemohon')->first(),
            ...
            'if_kekayaan_simpanan' => FormPprDataKekayaanSimpanan::select('deleted_at')->where('form_ppr_pembiayaan_id', $id)->first(),

            'kekayaan_tanah_bangunans' => FormPprDataKekayaanTanahBangunan::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_kekayaan_tanah_bangunan' => FormPprDataKekayaanTanahBangunan::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'kekayaan_kendaraans' => FormPprDataKekayaanKendaraan::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_kekayaan_kendaraan' => FormPprDataKekayaanKendaraan::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'kekayaan_sahams' => FormPprDataKekayaanSaham::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_kekayaan_saham' => FormPprDataKekayaanSaham::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'kekayaan_lainnyas' => FormPprDataKekayaanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_kekayaan_lainnya' => FormPprDataKekayaanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'pinjamans' => FormPprDataPinjaman::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_pinjaman' => FormPprDataPinjaman::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'pinjaman_kartu_kredits' => FormPprDataPinjamanKartuKredit::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_pinjaman_kartu_kredit' => FormPprDataPinjamanKartuKredit::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            'pinjaman_lainnyas' => FormPprDataPinjamanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->get(),
            'if_pinjaman_lainnya' => FormPprDataPinjamanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            //Untuk menampilkan selected value
            //Fixed income
            'character' => PprCharacter::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'capital' => PprCapital::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'capacity' => PprCapacity::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'condition' => PprCondition::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'collateral' => PprCollateral::select()->where('form_ppr_pembiayaan_id', $id)->first(),

            //Non Fixed Income
            'characterNf' => pprCharacterNonFixed::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'capacityNf' => PprCapacityNonFixed::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'conditionNf' => PprConditionNonFixed::select()->where('form_ppr_pembiayaan_id', $id)->first(),
            'collateralNf' => PprCollateralNonFixed::select()->where('form_ppr_pembiayaan_id', $id)->first(),
    ...
    public function update(Request $request, $id)
    {
        ...
        $mulaiBekerja = Carbon::createFromFormat('d-m-Y', $request->form_pekerjaan_pemohon_mulai_bekerja)->format('Y-m-d');
        $tglPenerbitanAgunan1 = Carbon::createFromFormat('d-m-Y', $request->form_agunan_1_no_sertifikat_tgl_penerbitan)->format('Y-m-d');

        //Jika ada pasangan
        if ($request->form_pribadi_pemohon_status_pernikahan == 'Menikah') {
            $tglLahirPasangan = Carbon::createFromFormat('d-m-Y', $request->form_pribadi_istri_suami_tempat_lahir)->format('Y-m-d');
            $mulaiBekerjaPasangan = Carbon::createFromFormat('d-m-Y', $request->form_pekerjaan_istri_suami_mulai_bekerja)->format('Y-m-d');
        } else {
            $tglLahirPasangan = NULL;
            $mulaiBekerjaPasangan = NULL;
        }

        //Jika ada agunan 2
        if ($request->form_agunan_2_no_sertifikat_tgl_penerbitan != NULL) {
            $tglPenerbitanAgunan2 = Carbon::createFromFormat('d-m-Y', $request->form_agunan_2_no_sertifikat_tgl_penerbitan)->format('Y-m-d');
        } else {
            $tglPenerbitanAgunan2 = NULL;
        }
        ...
        if ($pembiayaan->dilengkapi_ao != 'Telah dilengkapi') {
            ...
            FormPprDataPribadi::where('id', $id)
                ->update([
                    ...
                    'form_pribadi_pemohon_tanggal_lahir' => $tglLahir,
                    ...
                    'form_pribadi_istri_suami_tanggal_lahir' => $tglLahirPasangan,
            ...
            FormPprDataPekerjaan::where('form_ppr_data_pribadi_id', $id)
                ->update([
                    ...
                    'form_pekerjaan_pemohon_mulai_bekerja' => $mulaiBekerja,
                    ...
                    'form_pekerjaan_istri_suami_mulai_bekerja' => $mulaiBekerjaPasangan,
            ...
            FormPprDataAgunan::where('id', $id)
                ->update([
                    ...
                    'form_agunan_1_no_sertifikat_tgl_penerbitan' => $tglPenerbitanAgunan1,
                    ...
                    'form_agunan_2_no_sertifikat_tgl_penerbitan' => $tglPenerbitanAgunan2,
            ...
            FormPprPembiayaan::where('id', $id)
                ->update([
                    'dilengkapi_ao' => 'Telah dilengkapi',
                    'form_cl' => 'Telah diisi',
                    'form_score' => 'Butuh revisi',
                ]);
            ...
            if ($pembiayaan->jenis_nasabah == 'Fixed Income') {
                ...
                $atr_fixed_score = PprScoringAtrFixedIncome::select()->where('ppr_scoring_id', $id)->first();
                $wtr_fixed_score = PprScoringWtrFixedIncome::select()->where('ppr_scoring_id', $id)->first();
                $cc_fixed_score = PprScoringCollateralFixedIncome::select()->where('ppr_scoring_id', $id)->first();
                ...
                $atr_non_fixed_score = PprScoringAtrNonFixedIncome::select()->where('ppr_scoring_id', $id)->first();
                $wtr_non_fixed_score = PprScoringWtrNonFixedIncome::select()->where('ppr_scoring_id', $id)->first();
                $cc_non_fixed_score = PprScoringCollateralNonFixedIncome::select()->where('ppr_scoring_id', $id)->first();
        ...
        return redirect('/ppr/revisi/')->with('success', 'Proposal Berhasil Diperbarui!');
    }
    ...
    public function destroy($id)
    {
        if (request('delete_repeater') == 'Hapus Kekayaan Simpanan') {
            FormPprDataKekayaanSimpanan::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Kekayaan Simpanan Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Kekayaan Tanah & Bangunan') {
            FormPprDataKekayaanTanahBangunan::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Kekayaan Tanah & Bangunan Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Kekayaan Kendaraan') {
            FormPprDataKekayaanKendaraan::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Kekayaan Kendaraan Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Kekayaan Saham') {
            FormPprDataKekayaanSaham::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Kekayaan Saham Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Kekayaan Lainnya') {
            FormPprDataKekayaanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Kekayaan Lainnya Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Pinjaman') {
            FormPprDataPinjaman::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Pinjaman Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Pinjaman Kartu Kredit') {
            FormPprDataPinjamanKartuKredit::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Pinjaman Kartu Kredit Berhasil Dihapus!');
        } elseif (request('delete_repeater') == 'Hapus Pinjaman Lainnya') {
            FormPprDataPinjamanLainnya::select()->where('form_ppr_pembiayaan_id', $id)->delete();
            return redirect('/ppr/revisi/' . $id . '/edit')->with('success', 'Data Pinjaman Lainnya Berhasil Dihapus!');
        } else {
        }
    }
}
