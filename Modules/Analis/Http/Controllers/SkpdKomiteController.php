...
class SkpdKomiteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->status_id == 11) {
            return redirect('/analis/skpd/komite/')->with('success', 'Proposal Berhasil Direkomendasikan!');
    ...
    public function show($id)
    {
        ...
        if ($data->skpd_golongan_id == 18) {
            $dsr = $total_pemasukan > 0
                ? number_format($angsuran / $total_pemasukan * 100)
                : 0;
        } else {
            $dsr = $gaji_tpp > 0
                ? number_format($angsuran / $gaji_tpp * 100)
                : 0;
        }
