...
class PprNasabahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $proposals = FormPprDataPribadi::when($search, fn($q) => $q->where('form_pribadi_pemohon_nama_lengkap', 'like', "%$search%")->orWhere('form_pribadi_pemohon_no_ktp', 'like', "%$search%"))
            ->paginate(10)
            ->withQueryString();
        return view('analis::ppr.nasabah.index', [
    ...
    {
        $data = FormPprPembiayaan::select()->where('form_ppr_data_pribadi_id', $id)->first();

        $nasabah = FormPprDataPribadi::select()->where('id', $id)->first();
        $pekerjaan_nasabah = FormPprDataPekerjaan::select()->where('form_ppr_data_pribadi_id', $id)->first();

        return view('analis::ppr.nasabah.lihat', [
            'title' => 'Nasabah',
            'pembiayaan' => FormPprPembiayaan::select()->where('form_ppr_data_pribadi_id', $id)->first(),
            'nasabah' => $nasabah,
