...
class UmkmNasabahController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $proposals = UmkmNasabah::when($search, fn($q) => $q->where('nama_nasabah', 'like', "%$search%")->orWhere('no_ktp', 'like', "%$search%"))
            ->paginate(10)
            ->withQueryString();
    ...
    public function show($id)
    {
        ...
        $jaminanByPembiayaan = UmkmJaminan::select()
            ->whereIn('umkm_pembiayaan_id', $datas->pluck('id'))
            ->get()
        ...
        $jaminanNamaByPembiayaan = $jaminanByPembiayaan->mapWithKeys(function ($jaminan) use ($jenisJaminanByKode) {
            ...
            return [$jaminan->umkm_pembiayaan_id => $namaJaminan];
        });
