...
class PasarProposalController extends Controller
{
    ...
    public function create()
    {
        ...
        $review = $latestHistories->whereIn('pasar_pembiayaan_id', $reviewIds)
    ...
    public function show($id)
    {
        ...
        $jaminanlain = PasarJaminanLain::select()->where('pasar_pembiayaan_id', $id)->first();
        $tenor = $data->tenor;
        $harga = $data->harga;
        $rate = $data->rate;
        ...
        if ($cekcicilanpasangan > 0) {
            $cicilanpasangan = PasarSlikPasangan::select()->where('pasar_pembiayaan_id', $id)->sum('angsuran');
        ...
        $historyStatus = PasarPembiayaanHistory::select()->where('pasar_pembiayaan_id', $id)->orderBy('created_at', 'desc')->first();
