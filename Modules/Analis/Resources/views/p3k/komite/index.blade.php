                            ...
                            <div class="card-datatable table-responsive pt-0">
                                ...
                                <tbody>
                                    @foreach ($proposals as $proposal)
                                        @php
                                            $proposal_p3k = $proposal->p3kPembiayaan; // Preloaded p3kPembiayaan
                                            $history = $proposal; // The latest history record (preloaded)
                                        @endphp

                                        @if ($proposal_p3k && $history->status_id == 11 && $history->jabatan_id == 3)
                                        <tr>
                                            <td></td>
                                            ...
                                            <td style="text-align: center">{{ $proposal_p3k->tenor }}</td>
                                            <td style="text-align: center">
                                                ...
                                                @elseif ($history?->statusHistory?->id ?? '' == 10)
                                                    <a class="badge rounded-pill badge-light-danger" data-bs-toggle="modal"
                                                        data-bs-target="#modalCatatanAkadBatal-{{ $history->id }}">
                                                        {{ $history?->statusHistory?->keterangan ?? '' }}
                                                    </a>
                                                @else
