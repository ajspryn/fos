...
class SkpdKomiteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('dokumen_deviasi')) {
            $dokumen_deviasi = $request->file('dokumen_deviasi')->store('skpd-dokumen_deviasi', 'public');
            if ($request->kategori_deviasi == 'DSR') {
                SkpdDeviasi::create([
                    'skpd_pembiayaan_id' => $request->skpd_pembiayaan_id,
                    'kategori_deviasi' => 'DSR',
                    'dokumen_deviasi' => $dokumen_deviasi,
                ]);

                return redirect('skpd/komite/' . $request->skpd_pembiayaan_id)->with('success', 'Dokumen Deviasi Berhasil Diupload!');
            } else if ($request->kategori_deviasi == 'Slik') {
                SkpdDeviasi::create([
                    'skpd_pembiayaan_id' => $request->skpd_pembiayaan_id,
                    'kategori_deviasi' => 'Slik',
                    'dokumen_deviasi' => $dokumen_deviasi,
                ]);

                return redirect('skpd/komite/' . $request->skpd_pembiayaan_id)->with('success', 'Dokumen Deviasi Berhasil Diupload!');
            } else if ($request->kategori_deviasi == 'Agunan') {
                SkpdDeviasi::create([
                    'skpd_pembiayaan_id' => $request->skpd_pembiayaan_id,
                    'kategori_deviasi' => 'Agunan',
                    'dokumen_deviasi' => $dokumen_deviasi,
                ]);
        ...
        if ($request->status_id == 3) {
            return redirect('skpd/komite')->with('success', 'Proposal Berhasil Diajukan!');
        } elseif ($request->status_id == 7) {
            return redirect('skpd/komite')->with('success', 'Proposal Diajukan Untuk Revisi!');
        } else {
        }
    }
    ...
    public function uploadBonMurabahah(Request $request)
    {
        if ($request->file('bon_murabahah')) {
            $bonMurabahah = $request->file('bon_murabahah')->store('foto-skpd-pembiayaan', 'public');
            SkpdFoto::create([
                'skpd_pembiayaan_id' => $request->skpd_pembiayaan_id,
