...
class SkpdController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = Cache::remember("skpd:dashboard:{$userId}:{$year}", 300, function () use ($userId, $year) {
            ...
            $diterima = DB::query()
                ...
                ->where('sh.jabatan_id', 4)
                ->count();

            $proposal = SkpdPembiayaan::whereNull('skpd_akad_id')
                ->where('user_id', $userId)
                ->count();

            $ditolak = SkpdPembiayaanHistory::where('status_id', 6)
                ->where('user_id', $userId)
                ->count();

            $review = SkpdPembiayaanHistory::where('status_id', 7)
                ->orderBy('created_at', 'desc')
                ->count();

            $pipeline1 = DB::query()
                ->fromSub($latestSub, 'sl2')
                ->join('skpd_pembiayaan_histories as sh2', 'sh2.id', '=', 'sl2.latest_id')
                ->join('skpd_pembiayaans as s2', 's2.id', '=', 'sl2.skpd_pembiayaan_id')
                ->where('s2.user_id', $userId)
                ->where('sh2.status_id', '!=', 6)
                ->where('sh2.status_id', '!=', 5)
                ->where('sh2.jabatan_id', '!=', 4)
                ->count();

            $cair = SkpdPembiayaan::join('skpd_pembiayaan_histories', 'skpd_pembiayaans.id', '=', 'skpd_pembiayaan_histories.skpd_pembiayaan_id')
                ->where('skpd_pembiayaans.user_id', $userId)
        ...
        $pasars = SkpdPembiayaan::selectRaw('count(id) as total, created_at')->where('user_id', $userId)->groupBy('created_at')->get();
        ...
        $data = SkpdPembiayaan::select('id', 'created_at')->where('user_id', $userId)->get()->groupBy(function ($data) {
