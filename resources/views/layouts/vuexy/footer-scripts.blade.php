...
@else
    ...
    <script src="{{ asset('assets/js/tables-datatables-basic.js') }}?v={{ filemtime(public_path('assets/js/tables-datatables-basic.js')) }}"></script>
    ...
    (function() {
        function normalizeSidebarBrandLinks() {
            document.querySelectorAll('.layout-menu .app-brand-link').forEach(function(brandLink) {
                ...
                if (!currentHref || (currentHref !== '/' && currentHref !== '{{ url('/') }}')) {
                    try {
